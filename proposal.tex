...
\RequestedDataRemark{
There are some JHKs pointings from the VISTA public survey VHS that cover our fields, but they are too shallow. In addition some parts of W1 are covered by VIDEO in ZYJHKs, which we would not have to repeat. Unfortunately the placement of these tiles is not optimal for mapping this VIPERS patch.

\medskip
We have also checked the CFHT WIRCAM archive. WIRCAM has a 4x smaller footprint than VIRCAM, but it has also been used to cover parts of VIPERS. Ks band coverage exists for VIPERS DR1, as well as somewhat too shallow Y band observations and sporadic shallow J and H images. We considered skipping Ks band in this proposal, but prefer to ensure instrumental homogeneity as well as the more robust photometric calibration that is possible with the larger VISTA footprint. Our VVDS fields have no WIRCAM coverage.
}
