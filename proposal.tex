...
We propose to obtain near-IR fluxes of some 100,000 galaxies in deep spectroscopic survey fields with VISTA. The rationale is two-fold: (i) obtain accurate SEDs for these galaxies enabling studies of stellar masses, stellar population ages and dust content; and (ii) establishing {\em the} reference data set for photometric redshift techniques that form the backbone of current and future weak lensing surveys. These measurements are particularly timely in view of the recent KiDS results (Hildebrandt et al. 2017) which find a significantly lower clustering amplitude of large-scale structure than what is predicted in Planck cosmology: the dominant systematic error on this result is the photometric redshift calibration, and as statistical errors come down this will become a fundamental limitation. Increasing the number of deep spectroscopically confirmed redshifts by a factor of ten will remove this bottleneck and allow weak lensing to set even stronger constraints on the cosmological model.
...
improve the overall quality of the photometric redshift estimates (used to define the tomographic bins) by including VISTA data from the ESO public VIKING survey. VIKING covers the same sky as KiDS, and was completed last year: its inclusion will enable us to define a fifth tomographic bin beyond redshift 0.9, which will increase the volume and hence the statistical power of KiDS by an estimated 20\%.
...
Bringing the CC accuracy to the level of DIR requires an increase in the area covered with deep spectra by a factor of 10, from 1.5 to 15 sq.deg. At the low redshift end we can use the GAMA survey, which overlaps with KiDS/VIKING observations, but beyond the GAMA redshift limit of 0.5 new data are required. The VIPERS survey is ideal for this purpose: its depth of I=22.5 is well-matched to KiDS, and its photometric preselection ensures that it contains mostly galaxies between redshifts 0.5 and 1.2 (Fig.~2).
...
\MakePicture{banana_DIR_vs_CC_vs_Planck.pdf}{height=8cm}
...
\MakePicture{ESO_prop_plot_zdist.pdf}{height=8cm}
\MakeCaption{Fig.~2: Numbers of spectroscopic redshifts available in GAMA, VIPERS and SDSS, vs. the number that could be used in the KiDS-450 CC calibration of Hildebrandt et al. (2017). GAMA overlaps with KiDS ans will be used at $z<0.5$; with the proposed KiDS/VIKING-like imaging of the VIPERS area we will be able to do the same at higher redshift. }
...
\WhyLunarPhase{We follow the VISTA VIKING survey constraints.}  
